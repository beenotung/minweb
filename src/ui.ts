...
function handleUI(req: Request, res: Response) {
...
<ul>
<li><a href="#" onclick="location.href=this.textContent">${origin}${baseUrl}/https://hk.yahoo.com/</a></li>
<li><a href="#" onclick="location.href=this.textContent">${origin}${baseUrl}/?theme=default&no_script=true&url=yahoo.hk</a></li>
</ul>
`),
  );
}
...
function handleProxy(req: Request, res: Response) {
  let url: string = req.url.replace(baseUrl, '');
  if (url[0] == '/') {
    url = url.substring(1);
  }
...
export function handle(req: Request, res: Response) {
  if (req.url.startsWith(baseUrl + '?') || req.url.startsWith(baseUrl + '/?')) {
