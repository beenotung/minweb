...
const is_name_char = (s: string) =>
  s != '=' && s != '/' && !is_space(s) && !is_bracket(s);
    ...
    switch (c) {
      ...
      default: {
        ...
        if (tagName === 'script') {
          for (
            ;
            offset < s.length && !compare_str(s, offset, '</script>');
            offset++
          ) {}
        } else {
...
export function parseHTMLTree(s: string): HTMLItem[] {
  ...
  parseHTMLText(s, 0, {
    ...
    onopentag: (name, attributes, noBody) => {
      ...
      if (
        !noBody &&
        ['img', 'br', 'hr', 'meta', 'link', 'input'].indexOf(
          name.toLowerCase(),
        ) !== -1
      ) {
        ...
        if (name !== currentLevel.tag.name) {
          ...
          if ('') {
            console.error({
              ...
              noBody,
              currentLevel,
              levelStack: levelStack.map(x => x.tag.name),
            });
...
export const tag_head_to_string = (
  name: string,
  attributes: Attribute[],
): string =>
...
export const htmlItem_to_string = (x: HTMLItem): string =>
  'command' in x
    ? `<!${tag_head_to_string(x.command.name, x.command.attributes)}>`
    : 'comment' in x
    ? `<!--${x.comment}-->`
    : 'tag' in x
    ? '<' +
      tag_head_to_string(x.tag.name, x.tag.attributes) +
      (x.tag.noBody && x.children.length === 0
        ? '/>'
        : '>' +
          x.children.map(htmlItem_to_string).join('') +
          `</${x.tag.name}>`)
    : 'text' in x
    ? x.text
    : (() => {
  ...
  'command' in x
    ...
    ? '<' +
      ...
      (x.tag.noBody && x.children.length === 0
        ? '/>'
        : '>' +
          x.children.map(htmlItem_to_string_no_comment).join('') +
          `</${x.tag.name}>`)
    : 'text' in x
    ? x.text
    : (() => {
